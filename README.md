...
reaching verify paths random bytes almost never hit. `ecdsa` also builds the
key from a chosen R, so verification recomputes infinity or an x at least the
group order, which no signer can reach. `group` builds its points as k·G from

...
def parse(path):
    """Returns (file, function, original, mutated, line) per record."""
    records, cur = [], None
    with open(path) as f:
...
def body(text, function):
    ...
    which must end in "{", to the first line that is just "}"."""
    head = re.compile(r"^(?:static|SECP256K1_INLINE)\b[^;\n]*\b%s\(.*\{$" % re.escape(function), re.M)
...
def mutate(secp, records):
    ...
    sites = {}
    for k, (path, function, original, mutated, line) in enumerate(records):
        where = "%s:%d" % (LIST, line)
        ...
        if len(hits) != 1:
            fail("%s: %r appears %d times in %s" % (where, original, len(hits), function))
...
def main():
    ...
    names = ["%s:%s: %s -> %s" % (path, function, original, m) for path, function, original, m, _ in records]
